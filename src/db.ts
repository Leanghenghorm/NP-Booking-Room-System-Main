...
export async function setupDb() {
  ...
  if (usersTableInfo && !usersTableInfo.sql.includes("'super_admin'")) {
    db.exec(`
      ...
      INSERT INTO users_new (id, full_name, email, password_hash, department_id, role, status, failed_login_count, locked_until, mfa_secret, mfa_enabled, created_at, auth_provider) 
      SELECT id, full_name, email, password_hash, department_id, role, status, failed_login_count, locked_until, mfa_secret, mfa_enabled, created_at, auth_provider FROM users;
      DROP TABLE users;
